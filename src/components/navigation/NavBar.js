    ...
    <>
            ...
            if (item.isPrimary) {
              return (
                <div key={item.name} className="relative -top-5">
                  <Link
                    href={item.href}
                    className="flex h-14 w-14 items-center justify-center rounded-full border-4 border-[#f3efe9] bg-[var(--brand)] shadow-xl shadow-black/20 transition-all active:scale-90"
      ...
      <nav className="fixed bottom-0 left-0 top-0 z-50 hidden w-20 flex-col border-r border-[var(--border)] bg-white transition-all duration-300 md:flex lg:w-64">
        <div className="mb-4 hidden items-center gap-3 p-6 lg:flex">
          <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-[var(--brand)] text-lg font-black text-white shadow-md">
            S
          </div>
          <span className="text-lg font-black tracking-tight text-[var(--ink)]">
            Spoken <span className="text-[var(--brand)]">Odyssey</span>
          </span>
        </div>
              ...
              return (
                <div key={item.name} className="my-3 w-full">
                  ...
                  >
                    ...
                    <span className="hidden text-sm font-black lg:block">Public Feed</span>
                  </Link>
                </div>
              );
            }
        ...
        >
          ...
          <div className="hidden flex-col overflow-hidden text-left lg:flex">
            <span className="truncate text-sm font-black text-[var(--ink)]">Alexander</span>
