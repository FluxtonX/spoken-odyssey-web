...
export default function AuthLayout({ children }) {
  const features = [
    ...
    {
      icon: <Heart size={14} className="text-indigo-600" />,
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] flex flex-col md:flex-row w-full text-slate-800 dark:text-slate-100 antialiased overflow-x-hidden">
      ...
      <div className="hidden md:flex md:w-1/2 bg-gradient-to-br from-[#f8fafc] via-[#f0f4f9] to-[#e2ecf8] text-slate-800 relative flex-col justify-between p-12 overflow-hidden border-r border-slate-200">
        
        {/* Decorative background glow circles */}
        <div className="absolute top-[-10%] right-[-10%] w-[60%] aspect-square rounded-full bg-indigo-400/15 blur-[100px] pointer-events-none" />
        <div className="absolute bottom-[-10%] left-[-10%] w-[60%] aspect-square rounded-full bg-blue-400/15 blur-[100px] pointer-events-none" />
        ...
        <div className="relative z-10 flex items-center gap-3">
          <Link href="/" className="flex items-center gap-2.5 hover:opacity-90 transition-opacity">
            ...
            <span className="text-lg font-black tracking-tight text-slate-800">
              Spoken <span className="text-indigo-600">Odyssey</span>
            </span>
          </Link>
        </div>

        {/* Central Graphic with Arch Mask */}
        <div className="relative flex flex-col items-center justify-center py-6 z-10">
          
          {/* Mirror / Arch Shaped Image Container */}
          <div className="relative rounded-t-full border-4 border-indigo-200 bg-white/60 p-2 shadow-xl backdrop-blur-md max-w-[260px] w-full aspect-[2/3] overflow-hidden transition-all duration-700 hover:border-indigo-400/50 hover:shadow-2xl">
          ...
          <div className="mt-8 text-center max-w-sm px-4">
            <h2 className="text-2xl font-black tracking-tight text-slate-800 mb-2">Capture life as it unfolds.</h2>
        ...
        <div className="relative z-10 grid grid-cols-3 gap-6 border-t border-slate-200 pt-8 mt-4">
          {features.map((feat, index) => (
            <div key={index} className="text-left">
              <div className="flex items-center gap-2 mb-1.5">
                ...
                <span className="text-[10px] font-black uppercase tracking-wider text-indigo-600">{feat.title}</span>
              </div>
              <p className="text-[10px] font-semibold text-slate-500 leading-normal">{feat.desc}</p>
            </div>
          ))}
        </div>
